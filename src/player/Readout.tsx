 ...
 *
 * A schedule can hold several voices, so this reports the *primary* one — the first audible
 * binaural voice — and names it when there is more than one. Per-voice values are already
 * available from the chart's crosshair, so repeating all of them here would be noise.
 */
...
function primaryVoice(schedule: Schedule): PrimaryVoice | null {
  const candidates = schedule.voices.filter(
    (voice) => voice.type === VoiceType.Binaural && !voice.hidden && voice.entries.length > 0,
  );
