...
/**
 * Export a program as audio or back to `.gnaural` (PLAN.md §5.1).
 *
 ...
 */
export function ExportPanel({ schedule }: ExportPanelProps) {
  const exporter = useExport(schedule);
    ...
    <section className="export">
      ...
      <div className="export__row">
        ...
        <label className="export__rate">
          ...
          <select
            value={exporter.sampleRate}
            disabled={busy}
            onChange={(event) => exporter.setSampleRate(Number(event.target.value))}
          >
